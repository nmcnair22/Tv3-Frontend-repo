<template>
  <div>
    ...
    <div v-if="incomeStatements && incomeStatements.length">
      <table>
        <thead>
          <tr>
            <th>Description</th>
...
async function fetchIncomeStatements() {
  try {
    console.log('Fetching income statements from backend');
    ...
    const url = `http://localhost:3000/income-statements${queryString ? '?' + queryString : ''}`;

    console.log('Full request URL:', url);

    const response = await axios.get(url);

    console.log('Income statements response:', response.data);

    // Ensure response.data.value is an array
    if (response.data && Array.isArray(response.data.value)) {
      incomeStatements.value = response.data.value;
    } else if (Array.isArray(response.data)) {
      // If the data is directly an array
      incomeStatements.value = response.data;
    } else {
      console.error('Unexpected data structure:', response.data);
      incomeStatements.value = [];
    }

    console.log('Updated incomeStatements:', incomeStatements.value);
  } catch (error) {
