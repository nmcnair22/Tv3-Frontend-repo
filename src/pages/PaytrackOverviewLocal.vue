<template>
    <div class="paytrack-overview">
      ...
      <div class="grid grid-cols-12 gap-4">
        ...
        <div class="col-span-12 md:col-span-6 lg:col-span-3">
          <StatBox
            :value="averageDSOComputed ? averageDSOComputed.toFixed(2) : '-'"
            title="Current DSO"
            format="number"
            :percentageChange="dsoChangeComputed"
      ...
      <div class="mt-8 flex flex-wrap -mx-2">
        ...
        <div class="w-full md:w-2/5 px-2 mb-6">
          ...
          <DataTable :value="perCustomerDSOComputed" class="min-w-full bg-white">
            <Column field="customerName" header="Customer Name" sortable></Column>
...
import { useDSOStore } from '../store/dsoStore'; // Import the DSO store
  ...
  const averageDSOComputed = computed(() => dsoStore.averageDSO);
  const previousDSOComputed = computed(() => dsoStore.previousDSO);
  const perCustomerDSOComputed = computed(() => dsoStore.perCustomerDSO);
  const dsoChangeComputed = computed(() => {
    if (previousDSOComputed.value === 0) {
      return averageDSOComputed.value === 0 ? 0 : 100;
    }
    return ((averageDSOComputed.value - previousDSOComputed.value) / previousDSOComputed.value) * 100;
  });
  ...
  function calculatePercentageLate() {
    const totalLate = agingStore.agingReport
      ...
      .reduce(
        (sum, item) =>
          sum + (item.period1Amount || 0) + (item.period2Amount || 0) + (item.period3Amount || 0),
        0
      );
  ...
  function calculateLateBreakdown() {
    ...
    const sumPeriod1 = filteredAgingReport.reduce(
      (sum, item) => sum + (item.period1Amount || 0),
      0
    );
    const sumPeriod2 = filteredAgingReport.reduce(
      (sum, item) => sum + (item.period2Amount || 0),
      0
    );
    const sumPeriod3 = filteredAgingReport.reduce(
      (sum, item) => sum + (item.period3Amount || 0),
      0
    );
  ...
  /**
   * Fetches DSO metrics for the current and previous periods using the DSO store.
  ...
  async function fetchDSOData() {
    try {
      ...
      console.log(`[${new Date().toLocaleTimeString()}] Fetched Current DSO:`, dsoStore.averageDSO);
  
      // Fetch DSO for previous period
      await dsoStore.fetchPreviousDSO(formattedStartDatePrevious, formattedEndDatePrevious);
      console.log(`[${new Date().toLocaleTimeString()}] Fetched Previous DSO:`, dsoStore.previousDSO);
    } catch (error) {
  ...
  /**
   * Fetches per-customer DSO data using the DSO store.
   */
  async function fetchPerCustomerDSOData() {
    try {
      ...
      console.log(`[${new Date().toLocaleTimeString()}] Fetched Per-Customer DSO:`, dsoStore.perCustomerDSO);
    } catch (error) {
  ...
  /**
   * Generates data for the Aging Summary chart.
   * @returns Chart data object.
   */
  ...
  /**
   * Fetches top late customers data.
   */
